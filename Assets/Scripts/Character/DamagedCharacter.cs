...
public class DamagedCharacter : MonoBehaviour
{
    ...
    {//와 이거지 이러려고 인터페이스 쓰는거지~ 개꿀~
        if (GetComponent<IBaseState>().health <= 0)
        {
            //Change to Ragdoll
            ChangeToRagdoll();
        }
    ...
    public void Hit(int damage, RaycastHit2D hit)//이거 나중에 static type...뭐 그런거 선언해서 쓰기 string말고
    {
        ...
        CallDamageFlash();
        IBaseState state = GetComponent<IBaseState>();
    ...
    private void HitEffect(IBaseState state, RaycastHit2D hit)
    {
